...
from queries.emit.hashmap_payload.blimpv import BlimpVHitmapEmitHashmapPayload
from src.simulators.result import RuntimeResult, HitmapResult, MemoryArrayResult
from src.configurations.hashables.blimp import BlimpSimpleHashSet, BlimpBucket, Hash32bitObjectNullPayload, Object32bit
...
class SimpleHashMap(BlimpSimpleHashSet):
    class SimpleBucket(BlimpBucket):
        class SimpleObject(Hash32bitObjectNullPayload):
            _PAYLOAD_OBJECT = Object32bit
        _KEY_PAYLOAD_OBJECT = SimpleObject
    _BUCKET_OBJECT = SimpleBucket


hash_map = SimpleHashMap(8, 128)
hash_map.insert(SimpleHashMap.SimpleBucket.SimpleObject(0, 999))
hash_map.insert(SimpleHashMap.SimpleBucket.SimpleObject(1, 998))
hash_map.insert(SimpleHashMap.SimpleBucket.SimpleObject(8191, 997))
hash_map.insert(SimpleHashMap.SimpleBucket.SimpleObject(8193, 996))

hitmap = HitmapResult.from_hitmap_byte_array(
    hitmap_byte_array=[128+64] + [0]*1022 + [1],
...
query = BlimpVHitmapEmitHashmapPayload(simulator=simulator, layout_configuration=layout_configuration)
