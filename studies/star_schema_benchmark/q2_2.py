...
class SSBQuery2p2(SSBQuery2pX):
    ...
    def _part_record_join_condition(self, record: SSBPartTable.TableRecord) -> bool:
        return SSBBrandEncoding.convert("MFGR#2221") <= record.brand <= SSBBrandEncoding.convert("MFGR#2228")
    ...
    def _validate(self, final_hitmap_result: HitmapResult, *args):
        ...
        for idx, record in enumerate(SSBPartTable(scale_factor=self.scale_factor, no_storage=True).records):
            if SSBBrandEncoding.convert("MFGR#2221") <= record.brand <= SSBBrandEncoding.convert("MFGR#2228"):
...
class SSBQuery2p2BlimpVSupplierPartDate(SSBQuery2pXSupplierPartDate, SSBQuery2p2):
    ...
    join_2_query_class = BlimpVHashmapEarlyTerminationHitmapPayloadJoin
...
class SSBQuery2p2BlimpSupplierPartDate(SSBQuery2pXSupplierPartDate, SSBQuery2p2):
    ...
    join_2_query_class = BlimpHashmapEarlyTerminationHitmapPayloadJoin
