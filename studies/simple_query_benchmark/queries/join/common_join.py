...
class BlimpHashMap(BlimpSimpleHashSet):
...
class BlimpVHashMap(BlimpSimpleHashSet):
...
class SQBCommonJoin(GenericSQBQuery):
    join_hash_table = GenericHashMap(0, 0)

    def _record_join_condition(self, record: tuple) -> bool:
        raise NotImplemented

    def _record_joined_hashtable_object(self, record: tuple):
        return Hash32bitObject8bPayload(record[0], record[1])

    def _build_hash_map(self):
        self.join_hash_table.reset()
        for idx, record in enumerate(self.db.b):
            if self._record_join_condition(record):
                self.join_hash_table.insert(self._record_joined_hashtable_object(record))
        return self.join_hash_table
    ...
    join_database_configuration_class = BlimpHitmapDatabaseConfiguration
    __join_database_configuration = None

    def _calculate_join_inout_size(self):
        return ceil_to_multiple(self.join_hash_table.size, self._get_hardware_config().row_buffer_size_bytes) + \
            ceil_to_multiple(  # selection against a_100, add 1% buffer for expected value
            int(self.db_a_size // self.parallelism_factor * (
                        (self._get_operation_1_args()["value"] + 1) / 100)),
    ...
    def _database_configuration_json(self):
        ...
        base["blimp_temporary_region_size_bytes"] = self._calculate_join_inout_size()
        return base

    def _get_join_database_configuration(self):
        if self.__join_database_configuration is None:
            self.__join_database_configuration = self.join_database_configuration_class(
                **self._database_configuration_json()
            )
        return self.__join_database_configuration

    __join_data_layout = None
    join_data_layout_configuration_class = BlimpIndexHitmapBankLayoutConfiguration

    def _get_join_record_limit(self):
        return self.db_a_size // self.parallelism_factor

    def _get_join_generator(self):
        return self.db.a_b_k_generator

    def _get_join_layout_configuration(self):
        if self.__join_data_layout is None:
            self.__join_data_layout = self.join_data_layout_configuration_class(
                hardware=self._get_hardware_config(),
                database=self._get_join_database_configuration(),
                generator=self._get_join_generator()
            )
        return self.__join_data_layout
    ...
    def _setup(self, **kwargs):
        ...
        assert self._get_join_layout_configuration().layout_metadata.total_records_processable == \
               self._get_join_record_limit(), "Join record set is too big for this parallelism factor"
    ...
    def _perform_operation_1_layout(self, *args):
        self._get_join_layout_configuration().perform_data_layout(self._get_bank_object())

    def _perform_operation_1_placement(self, *args):
        self._get_join_layout_configuration().reset_hitmap_index_to_value(self._get_bank_object(), True, 0)
    ...
    def _perform_operation_1_query(self, save_query_output: bool = False, save_runtime_output: bool = False) -> \
        ...
        kernel = self.operation_1_query_class(self._get_simulator(), self._get_join_layout_configuration())
        kernel_runtime, kernel_memory_array, kernel_hitmap = kernel.perform_operation(
            pi_element_size_bytes=self.db_index_size_bytes,
            hash_map=self.join_hash_table,
            hitmap_index=0,
            output_array_start_row=self._get_join_layout_configuration().row_mapping.blimp_temp_region[0] +
            math.ceil(self.join_hash_table.size / self._get_bank_object().hardware_configuration.row_buffer_size_bytes),
            output_index_size_bytes=1,
            **self._get_operation_1_args()
        )

        if save_query_output:
            kernel_hitmap.save('join.res')
        if save_runtime_output:
            kernel_runtime.save('join_runtime.res')
