...
class _BlimpVEarlyTerminationBitweaveHitmapEquality(
    Query[
        SimulatedBlimpVBank,
        Union[
            BlimpHitmapIndexBitweaveBankLayoutConfiguration
    ...
    def _perform_operation(
        ...
        """
        Perform a generic BLIMP-V EQUAL query operation.
        ...
        for h in range(rows_per_hitmap):
            ...
            for b in range(pi_element_size_bytes * 8):
                ...
                # let v1 be PI[bit]
                runtime += self.simulator.blimp_load_register(self.simulator.blimp_v1, row_to_check, return_labels)
            ...
            runtime += self.simulator.blimp_save_register(self.simulator.blimp_v2, hitmap_row, return_labels)
...
class BlimpVEarlyTerminationBitweaveHitmapEqual(_BlimpVEarlyTerminationBitweaveHitmapEquality):
...
class BlimpVEarlyTerminationBitweaveHitmapNotEqual(_BlimpVEarlyTerminationBitweaveHitmapEquality):
    def perform_operation(
            self,
            pi_subindex_offset_bytes: int,
            pi_element_size_bytes: int,
