...
class _BlimpAmbitHitmapEquality(
    Query[
    ...
    def _perform_operation(
        ...
        for h in range(rows_per_hitmap):
            ...
            if negate:
                ...
                runtime += self.simulator.blimp_ambit_dispatch(return_labels)  # Add another since invert does 2 copies
            runtime += self.simulator.blimp_cycle(2, "; inner loop return", return_labels)
...
class BlimpAmbitHitmapEqual(_BlimpAmbitHitmapEquality):
...
class BlimpAmbitHitmapNotEqual(_BlimpAmbitHitmapEquality):
    def perform_operation(
            self,
            pi_subindex_offset_bytes: int,
            pi_element_size_bytes: int,
