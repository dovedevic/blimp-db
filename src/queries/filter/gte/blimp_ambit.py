...
from src.simulators.blimp_ambit import SimulatedBlimpAmbitBank


class _BlimpAmbitHitmapGreaterThanOrEqual(
    Query[
    ...
    def _perform_operation(
        ...
        """
        Perform a generic BLIMP+AMBIT GREATER THAN OR EQUAL (>=) query operation assuming reserved space for hitmaps.
        ...
        for h in range(rows_per_hitmap):
            ...
            for b in range(pi_element_size_bytes * 8):
                ...
                runtime += self.simulator.blimp_ambit_dispatch(return_labels)
                if bit_at_value:
                    runtime += self.simulator.ambit_copy(
            ...
            runtime += self.simulator.blimp_cycle(2, "; inner loop return", return_labels)
        runtime += self.simulator.blimp_cycle(2, "; outer loop return", return_labels)
...
class BlimpAmbitHitmapGreaterThanOrEqual(_BlimpAmbitHitmapGreaterThanOrEqual):
    def perform_operation(
        ...
        """
        Perform a BLIMP+AMBIT GREATER THAN OR EQUAL (>=) query operation. If the PI/Key field is segmented, specify the
...
class BlimpAmbitHitmapInverseGreaterThanOrEqual(_BlimpAmbitHitmapGreaterThanOrEqual):
    def perform_operation(
            self,
            pi_subindex_offset_bytes: int,
            pi_element_size_bytes: int,
        ...
        """
        Perform a BLIMP+AMBIT INVERSE GREATER THAN OR EQUAL (!>=) query operation. If the PI/Key field is segmented,
