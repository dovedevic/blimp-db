...
class _BlimpBitweaveHitmapGreaterThanOrEqual(
    Query[
        SimulatedBlimpBank,
        Union[
            BlimpHitmapIndexBitweaveBankLayoutConfiguration
    ...
    def _perform_operation(
        ...
        """
        Perform a generic BLIMP GREATER THAN OR EQUAL (>=) query operation.
        ...
        for h in range(rows_per_hitmap):
            ...
            for b in range(pi_element_size_bytes * 8):
                ...
                # let v5 now be PI[bit] AND NOT value[bit]
                ...
                # let v5 be m_eq AND (PI[bit] AND NOT value[bit])
                ...
                # let m_gt be m_gt OR (m_eq AND (PI[bit] AND NOT value[bit]))
...
class BlimpBitweaveHitmapGreaterThanOrEqual(_BlimpBitweaveHitmapGreaterThanOrEqual):
    def perform_operation(
        ...
        """
        Perform a BLIMP GREATER THAN OR EQUAL (>=) query operation. If the PI/Key field is segmented, specify the
...
class BlimpBitweaveHitmapInverseGreaterThanOrEqual(_BlimpBitweaveHitmapGreaterThanOrEqual):
    def perform_operation(
            self,
            pi_subindex_offset_bytes: int,
            pi_element_size_bytes: int,
        ...
        """
        Perform a BLIMP INVERSE GREATER THAN OR EQUAL (!>=) query operation. If the PI/Key field is segmented, specify
