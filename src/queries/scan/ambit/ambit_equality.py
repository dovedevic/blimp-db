    ...
    def _perform_operation(
        ...
        for h in range(rows_per_hitmap):
            ...
            if negate:
                ...
                runtime += self.simulator.cpu_ambit_dispatch(return_labels)  # Have to add two here since invert does 2
            runtime += self.simulator.cpu_cycle(2, "; inner loop return", return_labels)
...
class AmbitHitmapEqual(_AmbitHitmapEquality):
...
class AmbitHitmapNotEqual(_AmbitHitmapEquality):
    def perform_operation(
            self,
            pi_subindex_offset_bytes: int,
            pi_element_size_bytes: int,
