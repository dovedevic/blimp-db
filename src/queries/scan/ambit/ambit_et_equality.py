...
class _AmbitETEquality(Query):
    ...
    def _perform_operation(
        ...
        """
        Perform a generic AMBIT Early Termination (ET) EQUAL query operation.
        ...
        for h in range(rows_per_hitmap):
            ...
            for b in range(pi_element_size_bytes * 8):
                ...
                # load the hitmap we just saved
                runtime += self.sim.blimp_load_register(self.sim.blimp_data_scratchpad, hitmap_row, return_labels)

                # What's the maximum size of bytes we can operate on in the processor
                bit_chunks = self.sim.configuration.hardware_configuration.row_buffer_size_bytes \
                    // (self.sim.configuration.hardware_configuration.processor_bit_architecture // 8)
                # Loop start/setup, per chunk [cmp(current_max, chunk+1),
                runtime += self.sim.blimp_cycle(
                    cycles=3 + bit_chunks * 4,
                    label="DS[0] <- MAX DS",
                    return_labels=return_labels
                )

                maximum = bitmanip.byte_array_to_int(self.sim.blimp_get_register(self.sim.blimp_data_scratchpad))
...
class AmbitETEqual(_AmbitETEquality):
    def perform_operation(
        ...
        """
        Perform an AMBIT Early Termination (ET) EQUAL query operation. If the PI/Key field is segmented, specify the
...
class AmbitETNotEqual(_AmbitETEquality):
    def perform_operation(
            self,
            pi_subindex_offset_bytes: int,
            pi_element_size_bytes: int,
        ...
        """
        Perform an AMBIT Early Termination (ET) NOTEQUAL query operation. If the PI/Key field is segmented, specify the
        segment offset and its size, as well as the value to check against. The value must be less than the maximum size
        expressed by the provided size. Return debug labels if specified.
